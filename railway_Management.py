...
def signup():
    ...
    while True:
        ...
        sql_cursor.execute(f"SELECT * FROM userdata WHERE Email='{SignUp_Email_Id}'")
        userfetch_data = sql_cursor.fetchone()
        if userfetch_data is None:
            while True:
            ...
            sql_cursor.execute(f"""INSERT INTO Userdata(Name,Email,Passwd) VALUES('{SignUp_Name}','{SignUp_Email_Id}','{hashed_pass}')""")
            database.commit()
            ...
            print(f"Email ID '{SignUp_Email_Id}' already Exist.")
            break
    print()
...
def book_ticket():
    ...
    try:
        ...
        if source == dest:
            ...
            if a.lower() == "y":
                sql_cursor.execute(f"""INSERT INTO ticketdata(User_ID, Source,Destination,Distance,Passengers,Price) VALUES({user_id},'{source}','{dest}',{dist_sum},{passengers},{cost})""")
                database.commit()
...
def check_tickets():
    ...
    if ticketfetch_data == []:
        ...
        for i in ticketfetch_data:
            sql_cursor.execute(f"SELECT * FROM userdata WHERE User_ID={user_id}")
...
def cancel_ticket():
    ...
    if ticketfetch_data == []:
        ...
        for i in ticketfetch_data:
            if i[0] == t:
                sql_cursor.execute(f"DELETE FROM ticketdata WHERE Tkt_ID={t}")
                database.commit()
...
Enter Your Choice: """)
    ...
    if user_id is not None:
        input_number = input("""1.) Book a Ticket
